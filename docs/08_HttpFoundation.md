...
1. Создаём класс `App\Controller\Api\v1\UploadController`
    ...
    {
        /**
         * @Route("/api/v1/upload", methods={"POST"})
         */
    ...
    ```
1. Выполняем запрос Upload file из Postman-коллекции v2, видим исходный путь к файлу в каталоге tmp
1. Заходим в контейнер `php` командой `docker exec -it php sh`. Дальнейшие команды выполняются из контейнера
1. Проверяем, что файл появился в каталоге `public/upload` с новым именем и исходным расширением.
...
1. Добавляем класс `App\Exception\DeprecatedApiException`
    ...
    ```
1. В классе `App\Controller\Api\v1\UserController` исправляем метод `saveUserAction`
    ```php
    /**
     * @Route("", methods={"POST"})
     */
...
1. В файл `config/services.yaml` добавляем
    ...
    ```
1. Выполняем запрос Add user из Postman-коллекции v2, видим код ответа 410 и наше сообщение об ошибке
...
1. Добавляем класс `App\Event\CreateUserEvent`
    ...
    ```
1. Добавляем класс `App\EventSubscriber\CreateUserEventSubscriber`
    ```php
    <?php
...
1. В классе `App\Controller\Api\v1\UserController`
    ...
    1. Добавляем метод `saveUserAsyncAction`
        ...
        ```
1. Выполняем запрос Add user async из Postman-коллекции v2, видим код ответа 202
1. Проверяем, что в БД появился новый пользователь